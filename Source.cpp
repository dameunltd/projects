...
int nScreenWidth = 80;        // Console Screen Size (columns)
    ...
    while (!bGameOver)
    {
        ...
        if (bForceDown)
        {
            if (DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX, nCurrentY + 1))
                nCurrentY++; // If it has space to go down, do it!
            else
            {
        ...
        if (!vLines.empty())
        {
            ...
            this_thread::sleep_for(400ms); // Delay 400ms
